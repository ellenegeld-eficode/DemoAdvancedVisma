...
- frontend-test: for frontend, UI, and browser automation tests.
- backend-test: for backend, API, and service tests.

Delegation protocol:
1. Detect test scope from the user request and repository contents.
2. If the scope is frontend-only, delegate to frontend-test.
3. If the scope is backend-only, delegate to backend-test.
